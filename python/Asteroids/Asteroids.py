...
class FlyingObject:
    ...
    def draw(self):
        arcade.draw_texture_rectangle(self.center.x, self.center.y,
                self.width, self.height, self.texture, self.angle, 255)
...
class Ship(FlyingObject):
    ...
    def destroyed(self):        
        super().__init__("images/ShipDestroyed.png")
        self.destroyed_angle = 0
        self.center.x = SCREEN_WIDTH / 2
        self.center.y = SCREEN_HEIGHT / 2
        self.alive = False
...
class Game(arcade.Window):
    ...
    def __init__(self, width, height):
        ...
        for i in range(INITIAL_ROCK_COUNT):
            LargeAst = LargeAsteroid()
            self.asteroids.append(LargeAst)
